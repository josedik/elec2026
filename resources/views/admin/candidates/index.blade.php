...
@section('content_header')
    <div class="alert alert-info">
        <h1>Search for candidates by district</h1>
    </div>
@stop
            ...
            <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" :config="$config" striped hoverable
                ...
                @foreach ($districts as $district)
                    <tr>
                        ...
                        <td>{{ $district->area }}</td>
                        <td class="tetx text-center">
                            @can('admin.candidates.index')
                            <a href="{{ route('admin.candidates.show', $district) }}"
                                class="btn btn-xs btn-default text-info mx-1 shadow" title="View candidates">
